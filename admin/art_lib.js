...
function record(theform) {
	...
	else if(elementExsists(theform,'selCat')==true && theform.selCat.selectedIndex==0)alert('Der skal vælges kategori.')
...
function donew(theform) {
	...
	if(document.validation==true) {
		theform.action='act_new_art.asp';
	}
}
...
function doedit(theform) {
	record(theform)
	if(document.validation==true) {
		theform.action='act_edit_art.asp';
	}
}
