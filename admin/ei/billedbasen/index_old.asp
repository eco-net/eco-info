...
rsPageData.Source = "SELECT i.kb,i.width,i.height,i.cat_id,i.size_id,i.id,i.filename,i.subtext,i.source,i.ext,c.name as catname,s.name as sizename  FROM (image_cat_maindata c INNER JOIN image_maindata i ON c.id = i.cat_id) INNER JOIN image_size s ON i.size_id = s.id  WHERE 0=0  ORDER BY i.filename;"
...
<%end if %>
<td> 
<img src="thumbnails/<%=(rsPageData.Fields.Item("id").Value)%>.jpg"> 
</td>
<td><span class="listheader"><%=(rsPageData.Fields.Item("id").Value)%> 
</span> <a href="#" onClick="MM_openBrWindow('imgwin.asp?id=<%=rsPageData.Fields.Item("id").Value%>','Billedet','scrollbars=yes,width=360,height=360')"><%=(rsPageData.Fields.Item("filename").Value)%></a> 
<br>
<%=(rsPageData.Fields.Item("width").Value)%> * <%=(rsPageData.Fields.Item("height").Value)%> 
, <%=(rsPageData.Fields.Item("kb").Value)%> kB<br>
<%=(rsPageData.Fields.Item("subtext").Value)%> , <%=(rsPageData.Fields.Item("source").Value)%></td>
<td align="right" class="sidebarHeader"> 
<div align="right"><%=(rsPageData.Fields.Item("sizename").Value)%> <br>
<%=(rsPageData.Fields.Item("catname").Value)%> <br>
</div>
</td>
</tr>
<%
