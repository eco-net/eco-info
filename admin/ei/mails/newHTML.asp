...
IF request("mail")=1 THEN
	theText="Nyheds-mail til Øko-info"
ELSEIF request("mail")=2 THEN
	theText="Mail til Insider-brugere"
ELSEIF request("mail")=3 THEN
	theText="Mail til modtagere fra Filemaker"
END IF
%>
...
<!--#include virtual="/admin/ei/inc_leftbar.asp" -->
<%if request("mail")=2 then %>
<p align="center"><a href="/admin/ei/mails/update.asp">Opdater Insidermail-adresser 
fra Organisationstabel</a></p>
<form name="form2" method="post" action="til_frameld_insider.asp">
<div align="center">Afmeld/Tilmeld insidermail for orgid:<br>
</div>
<div align="center">
<input type="text" name="id" value="orgid">
<input type="submit" name="Submit3" value="Afmeld">
<input type="submit" name="Submit2" value="Tilmeld">
</div>
</form>
<p>&nbsp;</p>
<% end if %>
...
<!--
function MM_test(dotest,theform) { //v2.0
	if(theform.sender.selectedIndex==0){
		alert('Afsender-adresse skal vælges !');
	} else {
		if(dotest==1)
...
<form name="form1" method="post" action="act_sendmailHTML.asp" onSubmit="if(this.headline.value.length==0 || this.body.value.length==0){alert('Begge felter skal udfyldes');return false};else return true;">
<b>Afsender-adresse:</b><br>
<select name="sender">
<option value="" selected>&lt;-- Vælg --&gt;</option>
<option value="nyhedsmail@eco-net.dk" <%if request("mail")=1 then response.write "selected" %>>nyhedsmail@eco-net.dk</option>
<option value="insider@eco-net.dk" <%if request("mail")=2 then response.write "selected" %>>insider@eco-net.dk</option>
<option value="eco-net@eco-net.dk" <%if request("mail")=2 then response.write "selected" %>>eco-net@eco-net.dk</option>
</select><br>

<span class="formLabeltext">Afsender navn:</span><br>
<input type="text" name="afsendernavn" class="formInputobjectLong">
<br>
<br>
<span class="formLabeltext">Emne:</span><br>
<input type="text" name="headline" class="formInputobjectLong">
<br>
<span class="formLabeltext">Tekst:</span><br>
<textarea name="EditorValue"  style="display: none;" class="formTextobjectBigger">
<!--#include file="newsmailtable.asp" -->
</textarea><!--include file="pswmailtable.asp" -->
<!--#include virtual="/shared/editor.asp" -->
<br>
<%IF request("mail")=2 THEN 
	response.write "Skriv #username#, #password# og #organisation# for at flette disse" &_
	" informationer ind i teksten.<br>"
END IF
%>
<br>

<input type="reset" name="Reset" value="Ryd" class="formbutton">
<input type="button" name="Submit" value="Send" class="formSubmitbutton" onClick="copyValue();MM_test(0,this.form);">
<input type="hidden" name="mail" value="<%=request("mail")%>">
<input type="hidden" name="test">
<input type="button" name="Submit4" value="Test" class="formSubmitbutton" onClick="copyValue();MM_test(1,this.form);">
</form>
</td>
</tr>
</table>
