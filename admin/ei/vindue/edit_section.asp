...
<%
'if request("delte")<>"" and request("delid")<>"" then
'	theid=request("delid")
'	theSQL="Delete FROM eitema_sektion WHERE id=" & theid
'	execCommand theSQL
'end if
%>
<%
...
If (CStr(Request("MM_insert")) <> "") Then
  ...
  MM_editTable = "eivindue_sektion"
  MM_editRedirectUrl = ""
  MM_fieldsStr  = "tema_mrk|value|nyt_mrk|value|link|value|linkdescr|value|linie|value|show|value|sponsor|value|tema_id|value|sek|value"
  MM_columnsStr = "tema_mrk|none,1,0|nyt_mrk|none,1,0|linktekst|',none,''|tekst|',none,''|linie|none,none,NULL|show|none,1,0|sponsor|',none,''|tema_id|none,none,NULL|sek|',none,''"
...
If (CStr(Request("MM_update")) <> "" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_editRedirectUrl = ""
  MM_fieldsStr  = "link|value|linkdescr|value|linie|value|show|value"
  MM_columnsStr = "linktekst|',none,''|tekst|',none,''|linie|none,none,NULL|show|none,1,0"
...
%>
<%
Dim rssekdgs__temaid
rssekdgs__temaid = "0"
if (request("id") <> "") then rssekdgs__temaid = request("id")
%>
<%
set rssekdgs = Server.CreateObject("ADODB.Recordset")
rssekdgs.ActiveConnection = MM_ecoinfo_STRING
rssekdgs.Source = "SELECT *  FROM eivindue_sektion  WHERE tema_id=" + Replace(rssekdgs__temaid, "'", "''") + " AND sek='dgs'  ORDER BY linie"
rssekdgs.CursorType = 0
rssekdgs.CursorLocation = 2
rssekdgs.LockType = 3
rssekdgs.Open()
rssekdgs_numRows = 0
%>
...
Dim Repeat1__index
Repeat1__index = 0
rssekdgs_numRows = rssekdgs_numRows + Repeat1__numRows
%>
...
<tr> 
<td bgcolor="#CCCCCC"> 
<p align="center">Find en link <br>
og kopier den til linien<br>
<br>
<input type="button" name="Submit2322" value="V&aelig;lg link" onClick="window.open('/shared/sel_link.asp?f=4','thewin','width=300,height=300,top=200,left=300')" class="formbutton">
</p>
<p align="center">link <br>
<input type="text" name="link" class="formInputobject">
<br>
tekst<br>
<input type="text" name="linkdescr" class="formInputobject">
<br>
<br>
</p>
</td>
...
<tr> 
<td>
<p><span class="listheader">Hj&aelig;lp!!!!!!<br>
<br>
</span><img src="/shared/graphics/layout/tema.gif" width="40" height="12"> afkryds 
n&aring;r linien tilh&oslash;rer temaet<br>
<img src="/shared/graphics/layout/nyt.gif" width="40" height="12"> afkryds for 
at fremh&aelig;ve en nyhed som ikke h&oslash;rer til temaet<br>
<span class="listheader">Afkryds ikke begge!!!<br>
<br>
Tekst </span>er den linie der vises p&aring; forsiden.<br>
N&aring;r <span class="listheader">Link </span>udfyldes bliver tekst skrevet som 
en link, ellers ikke.</p>
<p><span class="listheader">Linie </span>sorterer linierne.<br>
Linierne vises kun n&aring;r <span class="listheader">Vises</span> er afkrydset.</p>
<p><span class="listheader">Sponsor/annonc&oslash;r </span>er en intern oplysning 
om en betalende kunde. Er feltet udfyldt vises linkteksten fremh&aelig;vet</p>
</td>
</tr>
</table>
<br>
...
<tr> 
<td colspan="2" bgcolor="#CCCCCC"><span class="sidebarHeader">Rediger Vindue </span><span class="contentHeader2"><%=name%></span></td>
</tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr> 
<td valign="top"><span class="contentHeader2">Sektioner p&aring; Forside</span><br>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr> 
<td background="/shared/graphics/layout/dots_horz.gif"><img src="/shared/graphics/spacer.gif" width="3" height="1"></td>
</tr>
<tr height="20"> 
<td><span  class="homeHeader"><br>
De Gr&oslash;nne Sider </span><span class="footer">- det &oslash;kologiske modstykke 
til De Gule Sider</span><br>
<br>
<% If Not rssekdgs.EOF Or Not rssekdgs.BOF Then %>
<% 
While ((Repeat1__numRows <> 0) AND (NOT rssekdgs.EOF)) 
%>
<table border="0" cellpadding="0" cellspacing="0" width="100%">
<tr> 
<td> 
...
<tr> 
<td colspan="7" background="/shared/graphics/layout/dots_horz.gif" height="2"><img src="/shared/graphics/spacer.gif" width="3" height="1"></td>
</tr>
<tr height="20"> 
<td> 
...
<td> 
<div align="center"> link<br>
<input value="<%=(rssekdgs.Fields.Item("linktekst").Value)%>" type="text" name="link" class="formInputobject">
<br>
tekst<br>
<input value="<%=(rssekdgs.Fields.Item("tekst").Value)%>" type="text" name="linkdescr" class="formInputobjectLong">
</div>
</td>
<td> 
<div align="right"> </div>
...
<td><div align="center"> linie 
<input value="<%=(rssekdgs.Fields.Item("linie").Value)%>" type="text" name="linie" size="1" >
vises 
<input type="checkbox" name="show" value="checkbox" <%If rssekdgs.Fields.Item("show").Value = 1 Then Response.Write("CHECKED") : Response.Write("")%> >
<br>
...
<input type="hidden" name="MM_recordId" value="<%= rssekdgs.Fields.Item("id").Value %>">
</form>
</td>
<td> 
...
<p> 
<input type="hidden" name="hiddenField" value="<%=(rssekdgs.Fields.Item("id").Value)%>">
</p>
<p>&nbsp;</p>
...
Wend
%>
<% End If ' end Not rssekdgs.EOF Or NOT rssekdgs.BOF %>
...
<div align="center">NY LINIE</div>
</td>
</tr>
<tr height="20" bgcolor="#CCCCCC"> 
<td> 
<div align="center"><img src="/shared/graphics/layout/tema.gif"><br>
<input type="checkbox" name="tema_mrk" value="checkbox">
<br>
<img src="/shared/graphics/layout/nyt.gif" width="40" height="12"><br>
<input type="checkbox" name="nyt_mrk" value="checkbox">
</div>
</td>
<td> 
<div align="center">link<br>
...
<br>
tekst<br>
<input type="text" name="linkdescr" class="formInputobjectLong">
</div>
</td>
<td> 
<div align="center"> 
<p align="center">linie 
<input type="text" name="linie" size="1" >
vises 
<input type="checkbox" name="show" value="checkbox">
<br>
<br>
sponsor / annonc&oslash;r <br>
<input type="text" name="sponsor" size="20" class="formInputobject">
</p>
</div>
</td>
<td> 
<input type="hidden" name="tema_id" value="<%=id%>">
<input type="hidden" name="sek" value="dgs">
