...
function record(theform) {
	...
	else if(elementExsists(theform,'password')==true && (theform.password.value.length>8 || theform.password.value.length<4))alert('Adgangskode skal vre p mellem 4 og 8 tegn')
	else {
...
function donew(theform) {
	...
	if(document.validation==true) {
		theform.action='act_new_user.asp';
	}
}
...
function doedit(theform) {
	...
	if(document.validation==true) {
		...
		theform.action='act_edit_user.asp';
	}
}
