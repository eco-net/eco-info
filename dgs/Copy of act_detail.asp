...
'ok - events
rsData.source="SELECT m.id,m.title,m.startdate FROM eical_maindata m LEFT JOIN eical_r_org o ON m.id=o.calid WHERE o.orgid=" & rsPageData.Fields.Item("id").value & " AND m.validated=1 ORDER BY m.startdate DESC"
rsData.open()
...
' dgb - litterature
rsData.source="SELECT m.id,m.title,m.year FROM eilib_maindata m LEFT JOIN eilib_r_org o ON m.id=o.libid WHERE o.orgid=" & rsPageData.Fields.Item("id").value & " AND m.validated=1 ORDER BY year DESC"
rsData.open()
IF NOT rsData.EOF THEN
	i=0
