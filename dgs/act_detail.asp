...
ELSE
	...
	ELSEIF request("sektion" & eicount)="kur" THEN
		toolText="Tilbage til kurset i Kursus-kalenderen"
	END IF
...
rsData.source="SELECT m.id,m.title,m.startdate,o.orgid FROM eical_maindata m LEFT JOIN eical_r_org o ON m.id=o.calid WHERE o.orgid=" & rsPageData.Fields.Item("id").value & " AND m.startdate>=" & theDate & " ORDER BY m.startdate DESC"
rsData.open()
...
orgid=rsData("orgid")
	...
	WEND
	okList=okList & "</table>"
	okList=okList & "<a href='../ok/list.asp?orgid=" & orgid & "'>Alle arrangementer</a><br>"
END IF
rsData.close()
...
IF NOT rsData.EOF THEN
	...
	courseList="<br><b>Kurser / Uddannelser:</b><br><table cellspacing=""0"" cellpadding=""0"" border=""0"">"
	WHILE NOT rsData.EOF
		courseList=courseList & "<tr valign=""top""><td nowrap>" & rsData.Fields.Item("startdate").value & "&nbsp;&nbsp;</td><td><a href=""/kurser/detail.asp?" &_
