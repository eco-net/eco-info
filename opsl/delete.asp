...
if (request.cookies("eifrontuserid")  <> "") then rsPageData__ColParam = request.cookies("eifrontuserid") 
userid=CInt(rsPageData__ColParam)
%>
...
if userid>0 then
	rsPageData.Source = "SELECT id,title  FROM eiopsl_maindata  WHERE userid=" + Replace(rsPageData__ColParam, "'", "''") + ""
else
	rsPageData.Source = "SELECT id,title+' ('+Cast(createdate as varchar(30))+')' as title FROM eiopsl_maindata"
end if
...
<br>
<table width="85%" border="0" cellspacing="0" cellpadding="0" background="/shared/graphics/spacer.gif" name="detailContents" align="center">
<tr> 
...
<br>
<table width="85%" border="0" cellspacing="0" cellpadding="1" bgcolor="#999999" align="center">
<tr>
<td>
<table width="100%" border="0" cellspacing="0" cellpadding="8" bgcolor="#FAFAF4">
<form action="act_delete.asp">
<tr>
<td>
<% If Not rsPageData.EOF Or Not rsPageData.BOF Then %>

V&aelig;lg det opslag der skal slettes:<br>
<br>
<div align="center">
<select name="id">
...
<option value="<%=(rsPageData.Fields.Item("id").Value)%>"><%=(rsPageData.Fields.Item("title").Value)%></option>
...
<br>
<br>
<input type="submit" name="Submit" value="Slet" class="formbutton">
</div>
<% End If ' end Not rsPageData.EOF Or NOT rsPageData.BOF %>
<% If rsPageData.EOF And rsPageData.BOF Then %>


Der er ikke registreret nogle opslag i dit navn.<br>
<br>
<div align="center">
<input type="submit" name="Submit" value="OK" class="formbutton" onClick="this.form.action='index.asp';this.form.submit();">
</div>
