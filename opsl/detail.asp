...
rsPageData.Source = "SELECT *  FROM eiopsl_maindata m  WHERE 0=0 ORDER BY m.createdate DESC"
rsPageData.Source=replace(rsPageData.Source,"0=0",theWhere)
rsPageData.Source=replace(rsPageData.Source,"eiopsl_maindata m",theFrom)
...
<%=rsPageData.Fields.Item("title").Value%>
</td>
</tr>
<tr> 
...
<tr>
<td valign="top">
<p><%=replace(rsPageData.Fields.Item("kort_beskrivelse").Value & "",chr(13),"<br>")%></p>
<p><%=replace(rsPageData.Fields.Item("lang_beskrivelse").Value & "",chr(13),"<br>")%></p>
<%IF (rsPageData.Fields.Item("billede_filnavn").Value)<> "/log/ei/opsl/graphics/" AND (rsPageData.Fields.Item("billede_filnavn").Value)<> "" THEN %>
<img src="<%=picasaImgSize(rsPageData("billede_filnavn"),"/s400")%>" width="360" />
<% END IF%>
<p><span class="contentHeader2">Kategori</span><br>
<%=catList%></p>
</td>
</tr>
</table>
...
Oprettelsesdato</span><br>
<%
response.write FormatDateTime(rsPageData.Fields.Item("createdate").Value,vbLongDate)
%>
<br>
<br>
<span class="faktaboksheader">Indsender</span><br>
<%=(rsPageData.Fields.Item("Indsender").Value)%> 
<%if rsPageData.Fields.Item("postnr").Value > 0 then
response.write "<br>" & rsPageData.Fields.Item("postnr").Value & " " & rsPageData.Fields.Item("city").Value
end if %>
<br><br>
<span class="listheader">Kontakt</span> <br>
<% if rsPageData.Fields.Item("email").Value <>"" then %>
<a href="mailto:<%=(rsPageData.Fields.Item("email").Value)%>"><%=(rsPageData.Fields.Item("email").Value)%></a><br>
<%end if %>
<%if LEN(rsPageData.Fields.Item("telefon").Value)>0 then
response.write rsPageData.Fields.Item("telefon").Value
end if %>
