...
'rsPageData.Source = "SELECT m.id,m.title,m.kort_beskrivelse,p.city FROM (eiopsl_maindata m LEFT JOIN eisys_postnr p ON m.postnr=p.postnr) WHERE 0=0 ORDER BY m.createdate DESC"
rsPageData.Source = "SELECT m.id,m.cat_id,m.title,m.kort_beskrivelse,m.createdate,p.city,c.id,c.name  FROM (eiopsl_maindata m LEFT JOIN eisys_postnr p ON m.postnr=p.postnr)INNER JOIN eiopsl_cat_maindata c ON m.cat_id=c.id WHERE 0=0  ORDER BY m.createdate DESC"
rsPageData.Source=replace(rsPageData.Source,"0=0",theWhere)
'rsPageData.Source=replace(rsPageData.Source,"(eiopsl_maindata m LEFT JOIN eisys_postnr p ON m.postnr=p.postnr)",theFrom)
rsPageData.Source=replace(rsPageData.Source,"ORDER BY m.createdate DESC",theOrder)
...
<td class="contentHeader1" colspan="2" width="98%">Fandt <%=rsPageData_total%> opslag</td>
...
response.write "</a></span></td><td width=100><div align='right'>"%>
<%dato=(rsPageData.Fields.Item("createdate").Value)%>
<%=Datepart("d",dato)%>/<%=Datepart("m",dato)%>-<%=Datepart("yyyy",dato)%> 
<%response.write "</div></td></tr><tr><td>"

IF rsPageData.Fields.Item("city").value<>"" THEN response.write rsPageData.Fields.Item("city").value
response.write "<br>" & replace(rsPageData.Fields.Item("kort_beskrivelse").Value & "",chr(13),"<br>") 
response.write "</td><td><div align='right' class='listheader'>"
response.write "<a href='list.asp?opslcat=" & (rsPageData.Fields.Item("cat_id").Value) & "&opslname=" & (rsPageData.Fields.Item("name").Value) & "'>" 
response.write (rsPageData.Fields.Item("name").Value) & "</a>"
...
<td class="contentHeader1" height="22">Fandt ingen opslag</td>
<td width="8"><br>
</td>
</tr>
<tr> 
...
<br>
<br>
Der er ingen opslag på Opslagstavlen, der matcher din søgning.<br>
...
<!--#include virtual="/log/ei/opsl/inc_annoncer.asp"-->
</td>
</tr>
</table>
<!-- #EndEditable --> </td>
