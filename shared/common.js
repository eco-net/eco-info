function doSearch(theform) { //execute the relevant pages when generel search are used
	...
	if (theval==1) //searching all apps
		...
		{theform.thetext.name='opsltext';theform.action='/opsl/list.asp'}
...
function mailcolleague(){
 window.open('/home/mailpage.asp','mailpage','width=350,height=200,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,copyhistory=no,left=200,top=200')
 }
