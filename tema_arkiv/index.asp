...
rsTema.Source = "SELECT *  FROM eitema_maindata  WHERE id<>17"
rsTema.CursorType = 0
...
<td valign="top"> 
<p>&nbsp;</p>
<p class="contentHeader1">TEMAARKIV</p>
<% 
While ((Repeat1__numRows <> 0) AND (NOT rsTema.EOF)) 
%>
<% if (rsTema.Fields.Item("name").Value)<>"Intet Tema" then %>
<p><span class="contentHeader1"><a href="tema<%=(rsTema.Fields.Item("id").Value)%>.asp?id=<%=(rsTema.Fields.Item("id").Value)%>"><%=(rsTema.Fields.Item("title").Value)%></a><br>
</span><%=(rsTema.Fields.Item("descr").Value)%><span class="listheader"><br>
</span><br>
tema start: <%=(rsTema.Fields.Item("start").Value)%><br>
tema slut: <%=(rsTema.Fields.Item("endday").Value)%></p>
<% end if %>
<% 
...
<td width="180" valign="top" name="rightbar" bgcolor="#ECECDD">&nbsp; </td>
</tr>
</table>
</td>
<td background="/shared/graphics/layout/dots_vert.gif" width="1" valign="top"><img src="/shared/graphics/layout/cover_dots.gif" width="1" height="18"></td>
